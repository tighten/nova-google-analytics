...
use Tightenco\NovaGoogleAnalytics\AvgTimeOnPage;

class AvgTimeOnPageMetricTest extends TestCase
{
    /** @test */
    public function it_can_return_results_for_one_day()
    {
        $response = (new AvgTimeOnPage)->calculate($this->requestWithQueryParams(['range' => '1']));
    ...
    {
        $response = (new AvgTimeOnPage)->calculate($this->requestWithQueryParams(['range' => 'Y']));
        $this->assertFalse(empty($response));
    }
    ...
    {
        $response = (new AvgTimeOnPage)->calculate($this->requestWithQueryParams(['range' => 'LW']));
        $this->assertFalse(empty($response));
    }
    ...
    {
        $response = (new AvgTimeOnPage)->calculate($this->requestWithQueryParams(['range' => 'LM']));
        $this->assertFalse(empty($response));
    }
    ...
    {
        $response = (new AvgTimeOnPage)->calculate($this->requestWithQueryParams(['range' => '7']));
        $this->assertFalse(empty($response));
    }
    ...
    {
        $response = (new AvgTimeOnPage)->calculate($this->requestWithQueryParams(['range' => '30']));
        $this->assertFalse(empty($response));
    }
